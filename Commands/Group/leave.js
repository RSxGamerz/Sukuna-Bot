...
module.exports = {
    ...
    start: async (
      ...
      var modStatus = await mku.findOne({id:m.sender}).then(async (user) => {
        ...
        console.log(error)
        //return Yaka.sendMessage(m.from, { text: `An internal error occurred while checking your mod status.` }, { quoted: m });
      });

      
      if (modStatus=="false"&&!isCreator && !isAdmin)  return Yaka.sendMessage(m.from, { text: 'Sorry, only *GroupAdmins* and *Mods* can use this command !' }, { quoted: m });
